...
const handlechange = async (e) => {
  ...
  const fetchnewcode = async () => {
    ...
    setCode(parsed.count)
    setBarcode(parsed.count)
  }
  ...
  useEffect(() => {
    fetchShops().then(async data => {
      setShops(data)
      setSelectedShop(data[0])
      fetchCategories(data[0]._id)
      let data2 = await fetch(apiaddress + '/customers/getcustomersbyshop', {
        headers: {
          'shopid': data[0]._id
        }
      })
      let parsed = await data2.json()
  ...
  const handleSubmitForm = async () => {
    let newProduct = {
      ...
      barcode,
      suplier: suplier._id,
      supliersGroup:selectedShop._id,
    ...
    if (productName.length < 3) {
      ...
      try {
        let response = await fetch(apiaddress + '/management/products/addproduct', {
          method: "POST",
          headers: {
        ...
        if (parsed.success) {
          ...
          setDeliverExpense('0')
          setCostPlusDelivery('')
          setDoesCostIncludesDeliveryExpense(true)
          setMarkup({ amount: '0', percentage: '0' })
          setPriceWithMarkup('')
          setTax({ amount: '0', percentage: '0' })
          setPriceWithTax('')
          setDoesSaleIncludeTax(false)
          setSale('')
          setQtyPerPiece('1')
          setReorder('1')
          setDescription('')
    ...
    <DefaultLayout>
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="flex flex-col gap-5.5 p-6.5">
          ...
          <button className='mx-auto p-3 bg-blue-700 text-white rounded-md' onChange={handlechange} onClick={handleSubmitForm}>Submit For Pictures UPload</button>
        </div>
      </div>
    </DefaultLayout>
    </Menu>
