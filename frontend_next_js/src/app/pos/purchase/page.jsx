...
const Page = () => {
    ...
    const linkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const billData = await fetchBills('purchase', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(billData);
    ...
    const unlinkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const newBills = await fetchBills('purchase', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(newBills)
    ...
    const handleShopChange = async (shopid) => {
        ...
        const billData = await fetchBills('purchase', user._id, 'pending', shopid, selectedDate);
        if (billData.length > 0) {
            ...
            const newBill = await createBill('purchase', user._id, 'pending', shopid, selectedDate);
            setBills(newBill);
    ...
    useEffect(() => {
        const initializeData = async () => {
            try {
                ...
                const billData = await fetchBills('purchase', user._id, 'pending', defaultShop._id, selectedDate);
                if (billData.length > 0) {
                    setBills(billData);
                    setSelectedBill(billData[0]);
                    ...
                    const newBill = await createBill('purchase', user._id, 'pending', defaultShop._id, selectedDate, customer && customer);
                    setBills(newBill);
                    setSelectedBill(newBill[0]);
    ...
    const handleFinalize = async () => {
        if (paidamount === (total.costexpense - total.discount)) {
            let finalObject = {
                paymentType,
                paidamount,
                splitedPayments,
            ...
            if (parsed.success) {
                ...
                if (billData.length > 0) {
                    ...
                    const newBill = await createBill('purchase', user._id, 'pending', selectedShop._id, selectedDate);
                    setBills(newBill);
                    setSelectedBill(newBill[0]);
                    let newlist = await getDocumentItems(newBill[0]._id)
                    setitemsList(newlist)
    ...
    if(user && user.permissions.includes("purchase")){
    return (
        <Menu >
            ...
            <div onKeyDown={enter}>
                    ...
                    <div className='absolute top-0 left-0 w-full min-h-screen bg-boxdark z-99999 p-4'>
                                    ...
                                    <div className="w-3/4 flex flex-col">
                                        ...
                                        <div className='text-right text-xl font-bold text-green-600'>
                                            ...
                                            <div className='flex space-x-8 text-white justify-end'>
                                                ...
                                                <h3 className='w-1/12'>{total.costexpense - total.discount}</h3>
                                            </div>
                        ...
                        <div className={'flex w-full h-screen items-center justify-center'}>
                            <div className='w-8/12 min-h-72'>
                                ...
                                <div className="flex">
                                    <h3 className='text-2xl font-bold text-green-600 border-b-2 mb-2 p-2 border-blue-600 w-1/2'>Discount</h3>
                                    <input ref={discRef} type='text' onChange={(e) => {
                                        ...
                                        setTotal({ ...total, discount: Number(x) })
                                    }} className='text-2xl bg-transparent outline-none font-bold text-green-600 border-b-2 mb-2 p-2 border-blue-600 w-1/2' value={total.discount} />
                                ...
                                {selectedBill && selectedBill.customer &&
                                    <>
                                        ...
                                        <div className='flex space-x-8 text-white justify-end'>
                                            ...
                                            <h3 className={`text-2xl font-bold border-b-2 mb-2 p-2 mr-0 border-blue-600 w-1/2 ${selectedBill.customer.balance - totalSum >= 0 ? 'text-green-600' : 'text-rose-600'}`}>{selectedBill.customer.balance - totalSum}</h3>
                                        </div>
                                    </>}
                ...
                <TopBar searchTerm={searchTerm} searchType={searchType} setSearchType={setSearchType} handleSearchChange={handleSearchChange} products={products} suggestions={suggestions} setSuggestions={setSuggestions} helper={helper} handleBlur={handleBlur} searchRef={searchRef} />
                <div className='w-full flex min-h-screen'>
                    <div className='w-9/12 pt-12 pb-36 text-sm min-h-screen bg-black'>
                        ...
                        {itemsList && itemsList.length > 0 && itemsList.map((item, key) => (
                            <div key={key} id={`item${item.productData.itemCode}`} onClick={(e) => { itemClick(item._id) }} className={`flex w-full items-center border-blue-600 ${selectedItem && selectedItem._id === item._id && 'bg-blue-600'} border-dotted border-b-2`}>
                    ...
                    <div className='w-3/12 fixed right-0 min-h-screen bg-boxdark max-h-screen shadow-lg border-l-2 border-slate-500'>
                        ...
                        <div className='w-full mx-auto flex justify-center items-center text-center'>
                            <IoIosAddCircleOutline onClick={async () => {
                                await createBill('purchase', user._id, 'pending', selectedShop._id, selectedDate, customer && customer);
                                const billData = await fetchBills('purchase', user._id, 'pending', selectedShop._id, selectedDate);
                                setBills(billData);
                            ...
                            <MdOutlineDeleteOutline onClick={async () => {
                                await deleteDocument(selectedBill._id, 'purchase', user._id, 'pending', selectedShop._id, formatDate(selectedDate))
                                const billData = await fetchBills('purchase', user._id, 'pending', selectedShop._id, selectedDate);
                                if (billData.length > 0) {
                                    setBills(billData);
                                    setSelectedBill(billData[0]);
