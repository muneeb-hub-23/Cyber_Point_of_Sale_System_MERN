...
const Page = () => {
    ...
    const [total, setTotal] = useState({ totalitems: 0, totalamount: 0, totalexpense: 0, totalcost: 0, costexpense: 0, discount: 0, billDiscount: 0 })
    ...
    const copyDiscount = async(method)=>{
        ...
        let data = await fetch(apiaddress+'/pos/copydiscount',{
            ...
            body:JSON.stringify({method,selecteddocument:selectedBill._id,ids:[customer._id]})
        })
        let parsed = await data.json()
        if(parsed.success){
            toast("Data Fetched")
    ...
    const calculateTotal = (itemsList) => {
        ...
        setTotal({
            ...
            costexpense: roundToTwo(costexpense),
            discount: roundToTwo(discount),
            billDiscount: roundToTwo(0) // Already 0, rounding is redundant but included for consistency
        });
    }
    ...
    const linkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const billData = await fetchBills('refund', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(billData);
    ...
    const unlinkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const newBills = await fetchBills('refund', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(newBills)
    ...
    const handleShopChange = async (shopid) => {
        ...
        const billData = await fetchBills('refund', user._id, 'pending', shopid, selectedDate);
        if (billData.length > 0) {
            ...
            const newBill = await createBill('refund', user._id, 'pending', shopid, selectedDate);
            setBills(newBill);
    ...
    useEffect(() => {
        const initializeData = async () => {
            try {
                ...
                const billData = await fetchBills('refund', user._id, 'pending', defaultShop._id, selectedDate);
                ...
                if (billData.length > 0) {
                    ...
                    const newBill = await createBill('refund', user._id, 'pending', defaultShop._id, selectedDate);
                    setBills(newBill);
                    setSelectedBill(newBill[0]);
    ...
    const handleFinalize = async () => {
        if (paidamount === (total.costexpense - total.discount - total.billDiscount)) {
            let finalObject = {
            ...
            let data = await fetch(apiaddress + '/pos/finalize/finalizerefund', {
                method: "POST",
                headers: {
                    'content-type': 'application/json'
                },
            ...
            if (parsed.success) {
                ...
                if (billData.length > 0) {
                    ...
                    const newBill = await createBill('refund', user._id, 'pending', selectedShop._id, selectedDate);
                    setBills(newBill);
                    setSelectedBill(newBill[0]);
                    let newlist = await getDocumentItems(newBill[0]._id)
                    setitemsList(newlist)
    ...
    const handleItemDiscount = async (item, type, amount) => {
...
if(user && user.permissions.includes("refund")){
    return (
        <Menu >
            ...
            <div className='fixed -top-[200vh] -left-[200vw] -z-999999'>
                <DefaultLayout />
            </div>
            <div onKeyDown={enter}>
                    ...
                    <div className='absolute top-0 left-0 w-full min-h-screen bg-boxdark z-99999 p-4'>
                                    ...
                                    <div className="w-3/4 flex flex-col">
                                        ...
                                        <div className='text-right text-xl font-bold text-green-600'>
                                            ...
                                            <div className='flex space-x-8 text-white justify-end'>
                                                <h3 className='w-2/12'>Total Bill</h3>
                                                <h3 className='w-1/12'>{total.costexpense}</h3>
                                            </div>
                                            <div className='flex space-x-8 text-white justify-end'>
                                                <h3 className='w-2/12'>Discount</h3>
                                                <h3 className='w-1/12'>{total.discount}</h3>
                                            </div>
                                            <div className='flex space-x-8 text-white justify-end'>
                                                <h3 className='w-2/12'>Current Bill</h3>
                                                <h3 className='w-1/12'>{total.costexpense - total.discount - total.billDiscount}</h3>
                                            </div>

                                            {splitedPayments.map((sp, key) => (
                                                <div key={key} className='flex space-x-8 text-green-600 justify-end'>
                                            ...
                                            <div className='flex space-x-8 text-white text-2xl justify-end'>
                                                <h3 className='w-2/12'>Paid Amount</h3>
                        ...
                        <div className={'flex w-full h-screen items-center justify-center'}>
                            <div className='w-8/12 min-h-72'>
                                ...
                                <div className="flex">
                                    <h3 className='text-2xl font-bold text-green-600 border-b-2 mb-2 p-2 border-blue-600 w-1/2'>Cart Discount</h3>
                                    <input ref={discRef} type='text' onChange={(e) => {
                                        let x = e.target.value
                                        let amounta = Number(x); // Convert to a number

                                        if (!isNaN(amounta)) {
                                            x = amounta; // If the value is a valid number, assign it to amount
                                        } else if (typeof x === 'string' && x.slice(-1) === '-') {
                                            x = Number(x.slice(0, -1)) * -1; // Remove the hyphen and convert the rest to a negative number
                                        } else {
                                            return; // If it's not a number and doesn't end with a hyphen, return (do nothing)
                                        }
                                        setTotal({ ...total, billDiscount: Number(x) })
                                    }} className='text-2xl bg-transparent outline-none font-bold text-green-600 border-b-2 mb-2 p-2 border-blue-600 w-1/2' value={total.billDiscount} />

                                </div>
                                <div className="flex">
                                    <h3 className='text-2xl font-bold text-white border-b-2 mb-2 p-2 border-blue-600 w-1/2'>Total Amount</h3>
                                    <h3 className='text-2xl font-bold text-white border-b-2 mb-2 p-2 border-blue-600 w-1/2'>{total.costexpense - total.discount - total.billDiscount}</h3>
                                </div>
                                ...
                                <div className="flex">
                                    <h3 className='text-2xl font-bold text-white border-b-2 mb-2 p-2 border-blue-600 w-1/2'>Total Paid Amount</h3>
                                ...
                                {selectedBill && selectedBill.customer &&
                                    <>
                                        ...
                                        <div className='flex space-x-8 text-white justify-end'>
                                            ...
                                            <h3 className={`text-2xl font-bold border-b-2 mb-2 p-2 mr-0 border-blue-600 w-1/2 ${Number(selectedBill.customer.balance) - Number(totalSum) >= 0 ? 'text-green-600' : 'text-rose-600'}`}>{Number(selectedBill.customer.balance) - Number(totalSum)}</h3>
                                        </div>
                                    </>}
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        if (!exists) {
                                                            if (p === 'Debit') {
                                                                !customer ?
                                                                    setCustomerSelecting(true)
                                                                    : setSplitedPayments([...splitedPayments, { name: p, amount: total.costexpense - total.discount }]);
                                                            } else {
                                                                setSplitedPayments([...splitedPayments, { name: p, amount: total.costexpense - total.discount }]);
                                                            }
                                                        }
                                ...
                                <div className='w-full flex space-x-8 text-center'>
                                    ...
                                    <button className='w-7/12 p-3 m-5 mx-auto rounded-md hover:scale-110 bg-green-600 text-white font-bold text-3xl' onClick={handleFinalize}>Process</button>
                                </div>
                            </div>
                ...
                <TopBar searchTerm={searchTerm} searchType={searchType} setSearchType={setSearchType} handleSearchChange={handleSearchChange} products={products} suggestions={suggestions} setSuggestions={setSuggestions} handleEnter={handleEnter} helper={helper} handleBlur={handleBlur} searchRef={searchRef} />
                <div className='w-full flex min-h-screen'>
                    <div className='w-9/12 pt-12 pb-36 text-sm min-h-screen bg-black'>
                        ...
                        {itemsList && itemsList.length > 0 && itemsList.map((item, key) => (
                            <div key={key} id={`item${item.productData.itemCode}`} onClick={(e) => { itemClick(item._id) }} className={`flex w-full ${item.discount.amount!==0 && 'bg-green-500'} items-center border-blue-600 ${selectedItem && selectedItem._id === item._id && 'bg-blue-600'} border-dotted border-b-2`}>
                                ...
                                <input onChange={(e) => { (handleItemDiscount(item, 'amount', Number(e.target.value))) }} className="w-1/12 bg-transparent p-2 text-center text-white" value={item.discount.amount} />
                                <input onChange={(e) => { (handleItemDiscount(item, 'percentage', Number(e.target.value))) }} className="w-1/12 p-2 bg-transparent text-center text-white" value={item.discount.percentage} />
                    ...
                    <div className='w-3/12 fixed right-0 min-h-screen bg-boxdark max-h-screen shadow-lg border-l-2 border-slate-500'>
                        ...
                        <div className='w-full mx-auto flex justify-center items-center text-center'>
                            <IoIosAddCircleOutline onClick={async () => {
                                ...
                                await createBill('refund', user._id, 'pending', selectedShop._id, selectedDate, customer && customer);
                                const billData = await fetchBills('refund', user._id, 'pending', selectedShop._id, selectedDate);
                                setBills(billData);
                            ...
                            <MdOutlineDeleteOutline onClick={async () => {
                                await deleteDocument(selectedBill._id, 'refund', user._id, 'pending', selectedShop._id, formatDate(selectedDate))
                                const billData = await fetchBills('refund', user._id, 'pending', selectedShop._id, selectedDate);
                                if (billData.length > 0) {
                                    setBills(billData);
                                    setSelectedBill(billData[0]);
                    ...
                    <div className='w-9/12 flex fixed text-md justify-between items-center bg-boxdark bottom-0 left-0 text-white border-slate-500 border-t-2 h-36'>
                        ...
                        <div className='w-1/3 p-1'>
                            ...
                            <div className='flex justify-around hover:shadow-[0_0_10px_rgba(236,72,153,0.6),0_0_20px_rgba(236,72,153,0.6)] transition-shadow duration-300 ml-auto text-green-500 font-bold text-md w-full p-1'>
                            ...
                            <div className='flex justify-around hover:shadow-[0_0_10px_rgba(236,72,153,0.6),0_0_20px_rgba(236,72,153,0.6)] transition-shadow duration-300 ml-auto text-green-500 font-bold text-md w-full p-1'>
                                ...
                                <div className='w-1/2 text-xl'>{total && total.discount}</div>
                            </div>
