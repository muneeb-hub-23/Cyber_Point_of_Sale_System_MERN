...
const Page = () => {
    ...
    const linkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const billData = await fetchBills('stockreturn', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(billData);
    ...
    const unlinkCustomer = async (customer) => {
        ...
        if (parsed.success) {
            ...
            const newBills = await fetchBills('stockreturn', user._id, 'pending', selectedShop._id, selectedDate);
            setBills(newBills)
    ...
    const handleShopChange = async (shopid) => {
        ...
        setCustomerList(b)
        const billData = await fetchBills('stockreturn', user._id, 'pending', shopid, selectedDate);
        if (billData.length > 0) {
            ...
            const newBill = await createBill('stockreturn', user._id, 'pending', shopid, selectedDate);
            setBills(newBill);
    ...
    useEffect(() => {
        const initializeData = async () => {
            try {
                ...
                const billData = await fetchBills('stockreturn', user._id, 'pending', defaultShop._id, selectedDate);
                if (billData.length > 0) {
                    setBills(billData);
                    setSelectedBill(billData[0]);
                    ...
                    const newBill = await createBill('stockreturn', user._id, 'pending', defaultShop._id, selectedDate, customer && customer);
                    setBills(newBill);
                    setSelectedBill(newBill[0]);
    ...
    const handleFinalize = async () => {
        ...
        let data = await fetch(apiaddress + '/pos/finalize/finalizestockreturn', {
            method: "POST",
            headers: {
                'content-type': 'application/json'
            },
        ...
        if (parsed.success) {
            ...
            if (billData.length > 0) {
                ...
                const newBill = await createBill('stockreturn', user._id, 'pending', selectedShop._id, selectedDate);
                setBills(newBill);
                setSelectedBill(newBill[0]);
                let newlist = await getDocumentItems(newBill[0]._id)
                setitemsList(newlist)
    ...
    if(user && user.permissions.includes("stockreturn")){
    return (
        <Menu >
        <div onKeyDown={enter}>
                    ...
                    <div className={'flex w-full h-screen items-center justify-center'}>
                        <div className='w-8/12 min-h-72'>
                            ...
                            {/* <div className="flex">
                                ...
                                <p className='text-2xl bg-transparent outline-none font-bold text-white border-b-2 mb-2 p-2 border-blue-600 w-1/2'>{!selectedBill.customer ? (paidamount > total.costexpense ? paidamount - total.costexpense : 0) : (paidamount > newbalance ? paidamount - newbalance : 0)} </p>
                            </div> */}
                ...
                <div className='w-3/12 fixed right-0 min-h-screen bg-boxdark max-h-screen shadow-lg border-l-2 border-slate-500'>
                    ...
                    <div className='w-full mx-auto flex justify-center items-center text-center'>
                            <IoIosAddCircleOutline onClick={async () => {
                                ...
                                await createBill('stockreturn', user._id, 'pending', selectedShop._id, selectedDate, customer && customer);
                                const billData = await fetchBills('stockreturn', user._id, 'pending', selectedShop._id, selectedDate);
                                setBills(billData);
                            ...
                            <MdOutlineDeleteOutline onClick={async () => {
                                await deleteDocument(selectedBill._id, 'stockreturn', user._id, 'pending', selectedShop._id, formatDate(selectedDate))
                                const billData = await fetchBills('stockreturn', user._id, 'pending', selectedShop._id, selectedDate);
                                if (billData.length > 0) {
                                    setBills(billData);
                                    setSelectedBill(billData[0]);
