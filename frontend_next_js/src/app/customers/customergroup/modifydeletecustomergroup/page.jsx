...
const CustomerGroupPage = () => {
  ...
  const handleDelete = async (groupId) => {
    ...
    if (confirmation && confirmation.toLowerCase() === "delete") {
      try {
        const response = await fetch(`${apiaddress}/customers/deletecustomergroup/${groupId}`, {
          method: "DELETE",
          headers: {
            token,
          },
        ...
        if (data.success) {
          ...
          // Refresh the customer groups list
          fetchCustomerGroups();
        } else {
          toast.error("Failed to delete customer group");
        }
      } catch (error) {
        toast.error("Error occurred while deleting customer group");
      }
    } else {
      toast.error("Delete action cancelled or incorrect input.");
    }
  };

  // Handle modify action
  const handleModify = (groupId) => {
    router.push(`/customers/customergroup/modifydeletecustomergroup/${groupId}`);
  };

  if (user && user.permissions.includes("modifydeletecustomergroup")) {
    return (
      <Menu>
      <DefaultLayout>
        <ToastContainer />
                    ...
                    return (
                      <tr key={group._id} className="border-b bg-gray-900">
                        ...
                        <td className="p-3 text-center">
                          <button
                            onClick={() => handleModify(group._id)}
